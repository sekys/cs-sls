...
if(!$page) { header("Location: ../index.php"); exit; }
	...
	if($_POST['datum'] or $_POST['mapa'])
	{
		...
		$mapa = SLS::mysql_vstup(trim($_POST['mapa']));
		...
		$sql_pocet = SLS::mysql_count2("SELECT COUNT(id) as pocet FROM `phpbanlist`.`acp_vyzva` 
											WHERE prijal IS NULL AND sukromna = 0 AND
												(( ".$prikaz." )
												OR `mapa` LIKE '%".$mapa."%')");						
	} else {
		@$sql_vyzva=SLS::mysql_dbquery("SELECT * FROM `phpbanlist`.`acp_vyzva` c
									...
									ORDER BY datum ".SLS::get_zoznam()."");
		$sql_pocet = SLS::mysql_count2("SELECT COUNT(id) as pocet FROM `phpbanlist`.`acp_vyzva` WHERE prijal IS NULL AND sukromna = 0");
	}
	...
	// Hladat	
		...
		{											
				...
				echo '
					<tr>
						<td class="cup_riadok" width="10" align="center" style="padding: 0px;">
							<a href="'.SLS::cesta(10).$datum.'/'.$hodina.'/'.$vyzva['server'].'/">
								<img title="Prija&#357; v&yacute;zvu ?" src="'.SLS::$adresy[3].'cup_ok.gif" alt="Prija&#357; v&yacute;zvu ?" width="20" height="20" border="0" align="absmiddle">
							</a>
						</td>	
						<td class="cup_riadok" width="100" align="center" >
							<a href="'.SLS::cesta(5).$vyzva['ziada'].'/">'.SLS::set_clan_meno($vyzva['meno']).'</a> 
						</td>	
			...
			$pocet = SLS::mysql_count2("SELECT COUNT(id) as pocet FROM `phpbanlist`.`acp_vyzva` WHERE prijal IS NULL AND sukromna > 0");
			echo '<tr>
					<td class="cup_riadok" width="40" align="center" colspan="6" >
