...
if(is_numeric($_GET['p1'])) {
	@$sql_zapas = SLS::mysql_dbquery("SELECT * FROM `phpbanlist`.`cup_zapas` WHERE id = '".SLS::mysql_vstup($_GET['p1'])."'");
	@$zapas = mysql_fetch_assoc($sql_zapas);
} elseif(SLS::is_adresa($p1, $p2, $p3)) {
	@$sql_zapas = SLS::mysql_dbquery("SELECT * FROM `phpbanlist`.`cup_zapas` WHERE 
	...
	// SPE team 
			...
			if($pocet) { $pocet--; }
				...
				echo'<tr><td colspan="5" height="20">&nbsp;</td></tr>
					<tr>
						<td colspan="5" height="20" class="cup_zapas_text_normal" >Stiahnu&#357; <a class="cup_zapas_text_normal" href="'.SLS::cesta(19).$zapas['id'].'/">DEMO</a> - <a href="/cup/admin/zapas/'.$zapas['id'].'/">Admin</a></td>
					</tr>
		...
		<br>';
			...
			if (true) {
				...
				showcomments("Z", 0, 0, $zapas['id'], SLS::adresa_na_seba());
			}
			
		} else {
			echo SLS::get_spravu('Z&aacute;pas nen&aacute;jden&yacute;.',1);
		}			
// 	++++++++++++++++++++++++++++++++++++++++++++ Seky`s Liga System ++++++++++++++++++++++++++++++++++++++++++++++++++	?>