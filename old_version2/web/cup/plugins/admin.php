...
function what_page()
{
	...
	if(SLS::is_hrac()) 
	{
		...
		{
			echo SLS::get_spravu(SLS::sprava('access_no'));
			return 0;
		}
	} else {
		return 0;
	...
	// Nastavime
	SLS::$user['cs_meno'] = 'Admin '.SLS::$user['user_name'];
...
function choose_page()
{
	...
	<table class="cup_body" width="520" align="center" cellpadding="0" cellspacing="0">
		<tr>
			<td align="center">
				<a href="'.SLS::adresa_na_seba().'clan/">
					<img src="'.SLS::$adresy[3].'admin/clan.png" alt="Clan Administr&aacute;cia " title="Clan Administr&aacute;cia " border="0" align="absmiddle">
				</a>
			</td>		
			<td align="center">
				<a href="'.SLS::adresa_na_seba().'hrac/">
					<img src="'.SLS::$adresy[3].'admin/player.png" alt="Hr&aacute;&#269; Administr&aacute;cia " title="Hr&aacute;&#269; Administr&aacute;cia " border="0" align="absmiddle">
				</a>
			</td>				
			<td align="center">
				<a href="'.SLS::adresa_na_seba().'zapas/">
					<img src="'.SLS::$adresy[3].'admin/player.png" alt="Z&aacute;pas Administr&aacute;cia " title="Z&aacute;pas Administr&aacute;cia " border="0" align="absmiddle">
				</a>
			</td>	
		</tr>	
	</table>';
...
function clan_page()
{
	...
	// Mysql
	@$sql = mysql_query("SELECT * FROM `phpbanlist`.`acp_clans` WHERE id ='".SLS::mysql_vstup($p2)."'");
	$clan = mysql_fetch_assoc($sql);
	
	if(!$clan['id'])
	{
		echo SLS::get_spravu(SLSsprava('clan_nenajdeny'), 1);
		search_clan_page();
		return 0;
	}
	
	// Sam seba nemoze nastavovat
	if($clan['clan_id'] == SLS::$user['clan_id'])
	{
		echo SLS::get_spravu('Svoj clan nemozes administravovat.', 1);
		return 0;
	}
...
function zapas_page() {	
	global $p2;
	// Mysql
	@$sql = SLS::mysql_dbquery("SELECT * FROM `phpbanlist`.`cup_zapas` WHERE id ='".$p2."'");
	$zapas = mysql_fetch_assoc($sql);
	
	if(!$zapas['id']) {
		echo SLS::get_spravu('Zapas nenajdeny !', 1);;
		return 0;
	}
	...
	<form action="'.SLS::adresa_na_seba().'" method="post">
			...
			<div style="margin:5px;">	
				<table width="777" class="cup_zapas_bg" border="0" cellpadding="0" cellspacing="0">
					  <tr>
						<td height="30" class="cup_zapas_text_normal" id="cup_zapas_ciara_normal">&nbsp;'.SLS::$server[ $zapas['server'] ][0].' - '.date("Y-m-d \o H", $zapas['datum']).'hod</td>
	...
	// T team ,clan A		
			...
			@$sql_clan=SLS::mysql_dbquery("SELECT meno FROM `phpbanlist`.`acp_clans` WHERE id = '".$zapas['prijal']."'");
			$ziada = mysql_fetch_assoc($sql_clan);		  
			
			if($zapas['t_team']) {
				$team = explode(SLS::$str_parser_2, $zapas['t_team']);
			}
			...
			if($ziada['meno']) {
				$temp = '<a href="'.SLS::cesta(5).$zapas['prijal'].'/" ><span class="cup_zapas_text_t">'.SLS::mysql_vystup($ziada['meno']).'</span></a>';					
			} else {
				...
				{			
					...
					{							
						// meno ,kill,deatch
						$temp = explode(SLS::$str_parser_1, $team[$i]);	
						$temp[0] = trim($temp[0]);
						if(!is_numeric($temp[0]))	
						{								
							echo'<tr>
									<td height="20" class="cup_zapas_text_t" colspan="5">&nbsp;'.SLS::mysql_vystup($temp[0]).'</td>
									<td class="cup_zapas_text_t" align="right">'.$temp[1].'</td>
									<td class="cup_zapas_text_t" align="right">'.$temp[2].'</td>
									<td>&nbsp;</td>
								  </tr>';		
						} else {
							@$sql = SLS::mysql_dbquery("SELECT cs_meno FROM `cstrike`.`fusion_users` WHERE user_id ='".$temp[0]."'");
							$meno = mysql_fetch_assoc($sql);
							
							echo'<tr>
									<td height="20" colspan="5">&nbsp;<a href="'.SLS::$aplikacia.'profile.php?lookup='.$temp[0].'"><span class="cup_zapas_text_t">'.SLS::mysql_vystup( $meno['cs_meno'] ).'</span></a></td>
									<td class="cup_zapas_text_t" align="right">'.$temp[1].'</td>
									<td class="cup_zapas_text_t" align="right">'.$temp[2].'</td>
									<td>&nbsp;</td>
								  </tr>';
	...
	// CT team ,clan B		
			...
			@$sql_clan = SLS::mysql_dbquery("SELECT meno FROM `phpbanlist`.`acp_clans` WHERE id = '".$zapas['ziada']."'");
			$ziada = mysql_fetch_assoc($sql_clan);		  

			if($zapas['ct_team']) {
				$team = explode(SLS::$str_parser_2, $zapas['ct_team']);
			}
			...
			if( $ziada['meno'] ) {
				$temp = '<a href="'.SLS::cesta(5).$zapas['ziada'].'/" class="cup_zapas_text_ct" ><span class="cup_zapas_text_ct">'.SLS::mysql_vystup($ziada['meno']).'</span></a>';					
			} else {
				...
				{			
					...
					{							
						...
						{								
							// echo
							echo'<tr>
									<td height="20" class="cup_zapas_text_ct" colspan="5">&nbsp;'.SLS::mysql_vystup($temp[0]).'</td>
							...
							echo'<tr>
									<td height="20" colspan="5">&nbsp;<a href="'.SLS::$aplikacia.'profile.php?lookup='.$temp[0].'"><span class="cup_zapas_text_ct">'.SLS::mysql_vystup( $meno['cs_meno'] ).'</span></a></td>
									<td class="cup_zapas_text_ct" align="right">'.$temp[1].'</td>
									<td class="cup_zapas_text_ct" align="right">'.$temp[2].'</td>
									<td>&nbsp;</td>
								  </tr>';
	...
	// SPE team 				
			...
			if($zapas['spe_team']) {
				$temp = explode(SLS::$str_parser_2, $zapas['spe_team']);
			}
			...
			if($pocet) { $pocet--; }
				...
				{			
					...
					{														
							// echo
							echo'<tr>
									<td height="20" class="cup_zapas_text_spe" colspan="5">&nbsp;'.SLS::mysql_vystup($temp[$i]).'</td>
							...
							@$sql = SLS::mysql_dbquery("SELECT cs_meno FROM `cstrike`.`fusion_users` WHERE user_id ='".$temp[$i]."'");
							$meno = mysql_fetch_assoc($sql);
							if($meno['cs_meno']) {					  
							echo'<tr>
									<td height="20" class="cup_zapas_text_spe" colspan="5">&nbsp;<a href="profile.php?lookup='.$temp[$i].'"><span class="cup_zapas_text_spe">'.SLS::mysql_vystup( $meno['cs_meno'] ).'</span></a></td>
									<td class="cup_zapas_text_spe" align="center">&nbsp;</td>
									<td class="cup_zapas_text_spe" align="right">&nbsp;</td>
									<td class="cup_zapas_text_spe" align="right">&nbsp;</td>
									<td>&nbsp;</td>
				...
				echo' <tr><td colspan="5" height="20">&nbsp;</td></tr>
					  <tr><td colspan="5" height="20" class="cup_zapas_text_normal" >Stiahnu&#357; <a class="cup_zapas_text_normal" href="'.SLS::cesta(19).$zapas['id'].'/">DEMO</a></td></tr>
...
{	
	...
	if(!$clan['user_id'])
	{
		echo SLS::get_spravu(SLSsprava('hrac_no'), 1);
		search_player_page();
		return 0;
	}
	
	// Sam seba nemoze nastavovat
	if($clan['user_id'] == SLS::$user['user_id'])
	{
		echo SLS::get_spravu('Sam seba nemozes administravovat.', 1);
		return 0;
	}
...
function search_clan_page()
{
	...
	if($_POST['id'] and is_numeric($_POST['id'])) {
		$podmienka = " WHERE id = '".SLS::mysql_vstup(trim($_POST['id']))."' ";
	} elseif($_POST['meno']) {
		$podmienka = " WHERE `meno` LIKE '%".SLS::mysql_vstup(trim($_POST['meno']))."%' ";
	} else {
		echo '</table>';
		return 0;
	}
	// Mysql ...	
	$sql_pocet = SLS::mysql_count("SELECT COUNT(id) as pocet FROM `phpbanlist`.`acp_clans` ".$podmienka." ORDER BY bodov desc ");
	...
	if( $sql_pocet ) {
		...
		{ 
			echo '<tr>'; 
				echo '<td class="cup_riadok" width="10" align="center"><a href="'.SLS::adresa_na_seba().'" ><img src="'.SLS::$adresy[3].'/admin/tool.gif" alt="Upravi&#357; tento clan" title="Upravi&#357; tento clan" border="0" align="absmiddle"></a></td>';					
				echo "<td  class=\"cup_riadok\" width=\"300\" align=\"center\"><a href=\"".SLS::cesta(5).$row['id']."/\" >";			
...
function search_player_page()
{
	...
	if($_POST['id'] and is_numeric($_POST['id'])) {
		...
		$podmienka = " WHERE `cs_meno` LIKE '%".SLS::mysql_vstup(trim($_POST['meno']))."%' OR user_name LIKE '%".SLS::mysql_vstup(trim($_POST['meno']))."%'";
	} else {
		echo '</table>';
		return 0;
	}
	...
	{ 
		echo '<tr>'; 
			echo '<td class="cup_riadok" width="10%" align="center"><a href="'.SLS::adresa_na_seba().$row['user_id'].'/" ><img src="'.SLS::$adresy[3].'/admin/tool.gif" alt="Upravi&#357; tochto hraca" title="Upravi&#357; tochto hraca" border="0" align="absmiddle"></a></td>';					
			echo "<td  class=\"cup_riadok\" width=\"30%\" align=\"center\">
					<a href='/profile.php?lookup=".$row['id']."' >".$row['user_name']."</a>";
			echo "<td  class=\"cup_riadok\" width=\"30%\" align=\"center\">" . SLS::mysql_vystup($row['cs_meno']) . "</td>";
